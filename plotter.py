...
def load_data_from_csv(path, round):
    files = glob.glob(f'{path}experiment*globalRound-{round}.csv')
    dataframes = []
...
def plot(mean, std, round):
    metrics = ['reward[min]', 'reward[max]', 'reward[mean]',
    ...
    for metric in metrics:
        ...
        plt.xlabel('Time')
        plt.ylabel(metric)
        plt.savefig(f'charts/globalRound-{round}_{metric}.pdf')
        plt.close()
...
if __name__ == '__main__':
    ...
    for round in range(1, 11):
        dataframes = load_data_from_csv(data_path, round)
