...
/** Action associated to a device with battery consumption.
  *
  * Given a [batteryCapacity] in mAh, [deviceEnergyPerInstruction] in nJ, and [batteryVoltage] in V, this action will consume energy from the battery
  * based on the [softwareComponentsInstructions]. This action will also recharge the battery at a rate of [rechargeRate] C-rates.
  *
  * Initially, the battery is charged with [startupCharge] %. By default, it is 100%. By default, the [batteryVoltage] is 3.7 V, and the
  * [rechargeRate] is 0.5 C-rates.
  */
class BatteryEquippedDevice[T, P <: Position[P]](
    ...
    private val osInstructions: Long = 0L, // Instructions per OS component
  ...
  def this(
      ...
      softwareComponentsInstructions: Map[String, Long], // Instructions per software component
      startupCharge: Double, // Initial battery charge in %
      batteryVoltage: Double , // Battery voltage in V
      rechargeRate: Double, // Recharge rate in C-rates
  ) = this(environment, random, node, batteryCapacity, deviceEnergyPerInstruction, softwareComponentsInstructions, startupCharge, batteryVoltage, rechargeRate, 0L)
  ...
  private def dischargeLogic(deltaTime: Double): Unit = {
    ...
    val componentsConsumedEnergy = actualComponents
      .map { case (_, instructions) => instructions * epiInJoule }
      .reduceOption(_ + _)
      .getOrElse(0.0) + (math.abs(random.nextGaussian()) * osInstructions * epiInJoule)
